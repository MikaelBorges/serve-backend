...
app.use(express.json()); // sert à ce que req.body soit reconnu (la propriété body)
app.use(cors()); // Autorise toutes les front à taper l'api
//app.use(cors({origin: '*'}))

// Plus besoin maintenant de bodyParser
/* app.use(bodyParser.urlencoded({extended: false}))
app.use(bodyParser.json()) */

//ici on recup tout le dossier public (css, fonts, img, js)
...
//session va gérer la création/vérification du token lors du login : Pas obligé
app.use(
  session({
    secret: "secret",
    resave: false,
